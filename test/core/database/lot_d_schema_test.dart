...
import 'package:lot_d_sandbox/core/database/app_database.dart';
...
void main() {
  group('AppDatabase schema v3', () {
    ...
    test('recipe_images table exists and supports CRUD + cascade', () async {
      ...
      final photos = await (db.select(db.recipeImages)
      ...
      final stillThere = await (db.select(db.recipeImages)
            ..where((t) => t.recipeId.equals('r1')))
          .get();
    ...
    test('recipe_items supports ingredient_id FK (nullable)', () async {
      ...
      final item = await (db.select(db.recipeItems)
            ..where((t) => t.id.equals('ri1')))
          .getSingle();
